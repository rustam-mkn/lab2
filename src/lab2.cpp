...
<<<<<<< HEAD
    // Задание 3 выполняется в отдельном main файле
    // это решение принято исходя из продолжительного неудачного опыта рефакторинга проекта
=======
    /**
     ...
     * файл. Далее мы рассмотрим один из способов разделения такой логики.
                    ...
                    BaseFile (базовый класс)
                    │
                    ├── Base32File (кодирование в base32)
                    │
                    └── RleFile (сжатие методом RLE)

                    |
                    V

                    IProcessor (интерфейс обработки данных)
                    │
                    ├── Base32Processor (кодирование в base32)
                    │
                    └── RleProcessor (сжатие методом RLE)
                    │
                    └── (можно добавить другие процессоры)
                    
                    BaseFile (базовый класс для работы с файлами)
                    │
                    ├── DiskFile (чтение/запись файлов на диск)
                    │
                    └── MemoryFile (чтение/запись в строку или буфер)

                    FileWriter (использует BaseFile + IProcessor)
                    │
                    └── Можно комбинировать кодирование и сжатие
     */
    line();
